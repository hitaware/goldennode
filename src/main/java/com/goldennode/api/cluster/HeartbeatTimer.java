...
public class HeartbeatTimer {
	static org.slf4j.Logger LOGGER = LoggerFactory
			.getLogger(HeartbeatTimer.class);

	private Timer timer;
	private Cluster cluster;
	private static final int TASK_DELAY = Integer.parseInt(SystemUtils
			.getSystemProperty("1000",
					"com.goldennode.api.cluster.HeartbeatTimer.taskDelay"));
	private static final int TASK_PERIOD = Integer.parseInt(SystemUtils
			.getSystemProperty("2000",
	...
	public void schedule(final Server server,
			final HearbeatStatusListener listener) {
		timer.schedule(new TimerTask() {

			@Override
			public void run() {
				try {
					cluster.unicastTCP(server, new Operation(null, "ping",
