...
public class GoldenNodeGrid extends Grid {
	...
	public <K, V> Map<K, V> newReplicatedMemoryMapInstance() throws GridException {
		return newDistributedObjectInstance(ReplicatedMemoryMap.class);
	}
	...
	public <E> Set<E> newReplicatedMemorySetInstance() throws GridException {
		return newDistributedObjectInstance(ReplicatedMemorySet.class);
	}

	@SuppressWarnings("unchecked")
	@Override
	public <E> List<E> newReplicatedMemoryListInstance() throws GridException {
		return newDistributedObjectInstance(ReplicatedMemoryList.class);
	}
	...
	void peerIsDeadOperation(Peer peer) {
		...
		if (peer.isMaster()) {
			...
			/*
			 * NO NEED TO RESET FOR NOW heartBeatTimer.stop(); peerManager.clear();
			 * distributedObjectManager.clearRemoteObjects(); leaderSelector.reset();
			 * peerAnnounceTimer.schedule();
			 * LockHelper.sleep(GoldenNodeGrid.PEER_ANNOUNCING_DELAY);
			 * peerAnnounceTimer.stop(); leaderSelector.rejoinElection();
			 */
		} else {
		...
		try {
			...
			for (Peer remotePeer : peers) {
				try {
					...
					// LOGGER.error("Error occured while processing operation" + operation + "on
					// peer" + remotePeer
					// + e.toString());
					mr.addErroneusResponse(remotePeer, e);
