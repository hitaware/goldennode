...
public class TestGoldenNodeServer {
	static org.slf4j.Logger LOGGER = LoggerFactory
			.getLogger(TestGoldenNodeServer.class);
	...
	public void testBlockingMulticast() throws ServerException {
		...
		try {
			...
			server.start();
			Request r = server.prepareRequest("_getSum", new RequestOptions(),
			...
			r = server.prepareRequest("_echo", new RequestOptions(),
					"Hello ozgen");
			r.setTimeout(1000);
			l = server.blockingMulticast(r);
			Assert.assertEquals(l.size(), 1);
			Assert.assertNull(l.get(0).getReturnValue());
			r = server.prepareRequest("_getSumException", new RequestOptions(),
					new Integer(3), new Integer(4));
			r.setTimeout(1000);
			l = server.blockingMulticast(r);
			Assert.assertEquals(l.size(), 1);
	...
	public void testUnicastUDP2() throws ServerException {
		...
		try {
			...
			server.setOperationBase(proxy);
			server.start();
			Request r = server.prepareRequest("_getSumException",
					new RequestOptions(), new Integer(3), new Integer(4));
	...
	public void testUnicastTCP() throws ServerException {
		...
		try {
			...
			r = server.prepareRequest("_nullParamTest", new RequestOptions(),
					(Object) null);
			resp = server.unicastTCP(server, r);
